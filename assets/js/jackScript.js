...
document.querySelector('.sticky').addEventListener('wheel', function(e) {
    ...
    this.scrollBy({
        top: e.deltaY * 0.5, // Adjust this multiplier for smoother scroll
        behavior: 'smooth'
    });
});
...
<script>
    // Enable scroll on hover for `.comments-section`
    const commentBox = document.querySelector('.comments-section');

    commentBox.addEventListener('mouseenter', () => {
        commentBox.focus(); // Automatically focus to allow scroll without click
    });

    commentBox.addEventListener('mouseleave', () => {
        commentBox.blur(); // Remove focus when mouse leaves to keep UI clean
    });
</script>
